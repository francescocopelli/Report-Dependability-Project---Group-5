...
The VirusTotal JSON report provides additional evidence that reinforces and extends the findings described above. The explicit behavioural tags assigned by the platform (\texttt{sets-process-name}, \texttt{obfuscated}, \texttt{detect-debug-environment}, \texttt{checks-cpu-name}, \texttt{telephony}, \texttt{checks-gps}, \texttt{reflection}) confirm active anti-analysis capabilities that place this sample at the same evasion level as the third Malbus variant. The reputation score of \texttt{-2} and the dominant threat classification as ``trojan/downloader/dropper'' across 22 engines further consolidate the assessment. The variety of submission names (354 total submissions under aliases including \texttt{malbus.apk} and \texttt{bus.apk}) confirms deliberate repackaging and broad distribution.

In parallel, the dynamic analysis in Genymotion confirms that the app interacts with remote services and relies on background components and advertising frameworks, supporting the idea that some behaviour is driven at runtime rather than being fully visible from static inspection. Overall, the APK should be treated as unsafe. Even if part of its visible functionality appears legitimate, the combination of insecure communication, suspicious permissions, anti-analysis evasion, and evasive implementation choices shows a clear mismatch between the declared purpose of the app and its actual security posture.
