...
In addition to the permission analysis, VirusTotal assigns several behavioural and structural tags to the file: \texttt{obfuscated}, \texttt{telephony}, \texttt{reflection}, \texttt{checks-gps}, \texttt{android}, and \texttt{apk}. The tags \texttt{obfuscated} and \texttt{reflection} are consistent with the use of Java Reflection observed during static analysis, while \texttt{checks-gps} directly confirms that the application programmatically queries the GPS subsystem at runtime. The \texttt{telephony} tag further indicates interaction with device communication APIs, which is not expected for a bus-information application and may be relevant to tracking or device fingerprinting.
    ...
    \end{figure}
        ...
        Notably, the sandbox verdict provided by Zenbox Android classifies the sample as \texttt{CLEAN} with a confidence of 82\%. This result stands in contrast with the static detection rate of 40 out of 75 engines. The discrepancy is consistent with dropper-like evasion strategies: the malicious payload is not executed unconditionally at runtime, but may instead be triggered only under specific conditions --- such as receiving a remote command via GCM/FCM, or upon verification of environment properties retrieved from the AWS Elastic Beanstalk backend. This behaviour-dependent activation pattern makes sandbox-based dynamic analysis less reliable for this class of samples.
...
\hline
Sandbox evasion (Zenbox Android) & \texttt{CLEAN} verdict (82\%) despite 40/75 static detections; consistent with condition-dependent payload activation and dropper-like design. \\
\hline
...
The VirusTotal JSON report provides additional evidence that reinforces and extends the findings described above. The crowdsourced YARA match with the IcedID structural rule, the explicit behavioural tags assigned by the platform (\texttt{obfuscated}, \texttt{telephony}, \texttt{checks-gps}, \texttt{reflection}), and the discrepancy between the static detection rate (40/75 engines flagging the sample as malicious) and the Zenbox Android sandbox verdict (\texttt{CLEAN}, 82\% confidence) collectively point to a deliberate evasion design. The application is likely built to withhold its malicious payload under controlled execution environments, activating only upon receipt of a remote trigger --- a pattern consistent with advanced trojanised droppers rather than simple adware or opportunistic malware.
