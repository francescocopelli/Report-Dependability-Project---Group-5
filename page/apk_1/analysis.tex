...
\paragraph{What is it?} The \oner\  is a malicious Android application masquerading as a plugin for a transportation application series developed by a South Korean developer. The series provides a range of information for each region of South Korea, such as bus stop locations, bus arrival times and so on.
    \section{VirusTotal}
        ...
        \begin{itemize}
            \item 40 out of 75 security vendors flagged the file as ``\textbf{malicious}'';
            \item 28 out of 75 security vendors flagged the file as a ``\textbf{secure file}'';
            \item 7 out of 75 security vendors get an ``\textbf{Unable to process file type}'' error.
        \end{itemize}

        Most anti-malware programs correctly identify the sample as a generic Android trojan, but some of them are even able to recognise its malware family (downloader).
        ...
        VirusTotal assigns the sample a reputation score of \texttt{-1} and a suggested threat label of \texttt{trojan.andr/apkdownloader}. The popular threat classification aggregated across all detecting engines identifies ``trojan'' as the dominant category (18 engines), followed by ``downloader'' (5 engines). Notably, the engine Tencent identifies the sample under the specific name \texttt{a.privacy.MalBuslmh}, a designation that directly references the APK package name \texttt{lmh.android.cwbus}, confirming that this specific sample --- rather than a generic variant --- is known to the threat intelligence community.
        ...
        \end{figure}
            Here is a detailed analysis of the permissions (Fig.\ \ref{fig:virustotal_first_permissions}) which are required by the APK, declared in the Manifest file (Fig.\ \ref{fig:manifest_first}):
...
In addition to the permission analysis, VirusTotal assigns several behavioural and structural tags to the file: \texttt{obfuscated}, \texttt{telephony}, \texttt{reflection}, \texttt{checks-gps}, \texttt{android}, and \texttt{apk}. The tags \texttt{obfuscated} and \texttt{reflection} are consistent with the use of Java Reflection observed during static analysis, while \texttt{checks-gps} directly confirms that the application programmatically queries the GPS subsystem at runtime. The \texttt{telephony} tag further indicates interaction with device communication APIs, which is not expected for a bus-information application and may be relevant to tracking or device fingerprinting.
    ...
    \end{figure}
        ...
        VirusTotal also records several alternative names under which the sample has been submitted across its 377 total submissions: \texttt{malbus.apk}, \texttt{supplybus.apk}, \texttt{lmh.android.cwbus-1.0.3.apk}, and others. The variety of filenames under which the same hash appears suggests that the sample has been distributed or repackaged under different labels, which is consistent with trojanised application behaviour.

        Additionally, a notable anomaly is present in the \texttt{AndroidManifest}: the \texttt{GcmReceiver} component is registered under the category \texttt{lmh.android.dgbus}, which differs from the main application package \texttt{lmh.android.cwbus}. This discrepancy in package naming for a core push-messaging receiver may indicate the presence of a secondary plugin or dropper component operating under a separate namespace.
        ...
        Furthermore, a crowdsourced YARA rule match was recorded for this sample: the rule \texttt{IcedID}, from the ruleset \textit{IcedID} (author: \texttt{kevoreilly/threathive}, source: CAPEv2 on GitHub), matched the binary. While IcedID is historically associated with a Windows banking trojan, the matching rule in this context is a structural pattern rule targeting dropper and loader architectures. The match does not imply that the sample belongs to the IcedID family, but it does indicate that its internal structure shares characteristics with known advanced payload-delivery components, further reinforcing the dropper hypothesis.
...
public static final String FACEBOOK = "627046304133735228304842";
...
\hline
VT behavioural tags (\texttt{obfuscated}, \texttt{telephony}, \texttt{checks-gps}) & Confirm runtime evasion, device tracking via GPS, and telephony API interaction beyond the declared app purpose. \\\\
\hline
YARA match: \texttt{IcedID} rule & Structural similarity with known dropper/loader patterns detected via crowdsourced YARA; reinforces the hypothesis of an advanced payload-delivery architecture. \\\\
\hline
\end{longtable}
...
\end{figure}
MobSF highlights several security issues in the analyzed APK, confirming that the application exposes a broad attack surface.
...
MobSF reports one informational issue: the application logs information, including potentially sensitive data. This finding is classified as LOW severity in this variant; in \threer the same behaviour is elevated to HIGH severity due to a broader set of sensitive fields being exposed. It also reports two secure findings: the use of SSL communication and the presence of root detection capabilities. Finally, the scorecard flags one hotspot related to a critical permission, reinforcing the idea that the app is highly sensitive from a security perspective.
...
The combined results of VirusTotal, static analysis, MobSF, and Genymotion consistently indicate that \oner\ is suspicious and potentially malicious.
All four analytical tools converge on the same core patterns: unencrypted network traffic, aggressive integration of advertising libraries, remote configuration via \texttt{getEnvironmentProperties()}, exported components, and weaknesses related to storage and cryptography.

The static inspection reveals that the application extends well beyond a legitimate bus-information interface: the presence of a native library loaded at startup, native methods \texttt{startUpdate()} and \texttt{updateApplication()} consistent with dropper logic, and a cosmetic \texttt{AppInfoActivity} designed to disguise the application's true nature are all characteristic of repackaged or trojanised software.
MobSF reinforces this picture by reporting nine high-severity issues, including Janus vulnerability exposure, SHA-1 signing, StrandHogg weaknesses, hardcoded secrets, and multiple privacy trackers.
The dynamic analysis in Genymotion confirms that the app contacts remote services and relies on background components whose behaviour is driven at runtime rather than being fully encoded in the static bytecode.

At this stage, the evasion layer is limited to code obfuscation and Java Reflection --- the techniques recorded by the VirusTotal tags \texttt{obfuscated} and \texttt{reflection}.
No sandbox or debugger detection is present in this variant, which suggests that \oner\ represents the baseline form of the family before active anti-analysis hardening was introduced in later releases.
The APK should be treated as unsafe: the combination of insecure communication, suspicious permissions, and evasive implementation choices reveals a clear mismatch between the application's declared purpose and its actual security posture.
