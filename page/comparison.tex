...
Two differences emerge across versions. First, v1.0.3 additionally declares \texttt{WRITE\_HISTORY\_BOOKMARKS} and \texttt{READ\_GSERVICES}, which are absent in the later versions, suggesting a minor refinement of the permission scope in subsequent releases.
Second, v3.6.5 uniquely declares \texttt{CALL\_PHONE} and \texttt{RECORD\_AUDIO}, which are not present in v1.0.3 or v3.3.7, indicating an expanded capability to interact with device telephony and audio at the hardware level.
...
Notably, v3.6.5 received an explicit MobSF risk score of 38/100 (Grade C, HIGH RISK), while the other two samples did not produce a numerical score in the report.

\begin{table}[H]
\centering
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Finding (HIGH severity)} & \textbf{v1.0.3} & \textbf{v3.3.7} & \textbf{v3.6.5} \\
\hline
...
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Finding (MEDIUM severity)} & \textbf{v1.0.3} & \textbf{v3.3.7} & \textbf{v3.6.5} \\
\hline
...
The high-severity count is identical (9) across all three samples, but the composition differs slightly: v3.6.5 replaces the weak AdLib encryption finding with a native library hardening issue (\texttt{libAudio3.0} compiled without stack canary and PIE), and shows only StrandHogg 2.0 rather than the full Task Hijacking vulnerability present in v1.0.3 and v3.3.7.
At the medium-severity level, v3.6.5 uniquely introduces \texttt{CALL\_PHONE} and \texttt{RECORD\_AUDIO} permission warnings, consistent with its expanded hardware interaction surface observed in the static analysis.
...
The key evolution across versions is twofold: the progressive addition of anti-analysis and VM evasion capabilities (absent in v1.0.3, fully present in v3.3.7 and v3.6.5), and the expansion of hardware interaction permissions in v3.6.5 (\texttt{CALL\_PHONE}, \texttt{RECORD\_AUDIO}) combined with native library hardening weaknesses specific to that version.
